/**
 * @fileoverview Provides an XML reporter that generates Surefire like reports.
 *               Based on work by Ivan De Marino.
...
(function() {
	...
	/**
	 * PhantomJS Reporter generates JUnit XML for the given spec run. Allows the
	...
	PhantomJSReporter.prototype = {
		...
		reportSpecResults : function(spec) {
			...
			spec.output = '<testcase classname="' + this.getFullName(spec.suite) + '" name="'
		...
		reportSuiteResults : function(suite) {
			...
			var specOutput = "";
			// for JUnit results, let's only include directly failed tests (not
			// nested suites')
			var failedCount = 0;
			...
			// if a suite has no (active?) specs, reportSpecStarting is never
			// called
			...
			suite.output = '\n<testsuite name="' + this.getFullName(suite) + '" errors="0" tests="' + specs.length
					+ '" failures="' + failedCount + '" time="' + suite.duration + '" timestamp="'
					+ ISODateString(suite.startTime) + '">';
			suite.output += specOutput;
			suite.output += "\n</testsuite>";
			this.log(suite.description + ": " + results.passedCount + " of " + results.totalCount
					+ " expectations passed.");
		},

		reportRunnerResults : function(runner) {
			this.log("Runner Finished.");
			var suites = runner.suites(), passed = true;
			for ( var i = 0; i < suites.length; i++) {
				var suite = suites[i], filename = 'TEST-' + this.getFullName(suite, true) + '.xml', output = '<?xml version="1.0" encoding="UTF-8" ?>';
