 ...
 */
OpenLayers.Handler.KeyboardPoint = OpenLayers.Class(
		OpenLayers.Handler, {
			...
			initialize : function(control, callbacks, options) {
				OpenLayers.Handler.prototype.initialize.apply(
						this, arguments);
			...
			activate : function() {
				if (!OpenLayers.Handler.prototype.activate.apply(
						this, arguments)) {
					return false;
				}
			...
			deactivate : function() {
				if (!OpenLayers.Handler.prototype.deactivate.apply(
						this, arguments)) {
					return false;
				}
				for ( var i = 0, len = this.KEY_EVENTS.length; i < len; i++) {
			...
			modifyFeature : function(lon, lat) {
				...
				this.callback(
						"modify", [
								this.point.geometry, this.point, false
						]);
			...
			createFeature : function() {
				...
				this.callback(
						"create", [
								this.point.geometry, this.point
						]);
				this.point.geometry.clearBounds();
